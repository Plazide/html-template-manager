...
function templateWatch(template, static){
	...
	watcher.on("unlink", () => {
		console.error("Template file was removed... Exiting...");
		process.exit();
	});
}

function staticWatch(static, template){
	const exists = fs.existsSync(static);
	if(!exists)
		throw new Error().code = "ENOENT";
	...
	watcher.on("add", file => {
		simpleOverwrite(file, template);
	});

	watcher.on("unlinkDir", () => {
		throw new Error(`${static} was removed.`);
	});
...
function simpleOverwrite(file, template){
	...
	const instance = {
		startComment: `<!-- InstanceBegin template="${path.relative(path.dirname(file), template)}" codeOutsideHTMLIsLocked="false" -->`,
